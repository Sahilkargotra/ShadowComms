...
function UserDashboard (){
  ...
  },[setValue,toast]);  // added toast extra
  ...
  },[setIsLoading,setMessages,toast])  // added toast extra
...
if (typeof window !== 'undefined') {
 ...
 const profileUrl = `${baseUrl}/u/${username}`;
  ...
  {
    return <div className="text-4xl font-bold mb-4">
      Please Login
    </div>
  }
  return (
    ...
    />
      <span className ="ml-2">
    Accept Messages: {acceptMessages ? 'On' : 'Off'} </ span> </div>
    <Separator />
  ...
  <div className="mt-4 grid grid-cols-1 md:grid-cols-2
    ...
    {
      messages.length > 0?(
      messages.map((message) =>(     // index is  removed (message,index)
      <MessageCard
      key={message._id as string} // as string
      message={message}
